...
import Vd_icon from "../assets/vd-icon.png"; // ✅ fallback logo
...
const Navbar = () => {
  ...
  const [loading, setLoading] = useState(true); // ✅ track API load
    ...
    const fetchSettings = async () => {
      try {
        ...
        if (data.success && data.settings) {
          setSettings({
            companyName: data.settings.companyName || "Company",
  ...
  // ✅ Logo logic
  const logoToShow =
    !loading && settings.logoUrl ? settings.logoUrl : (!loading ? Vd_icon : "");
      ...
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        <div className="hidden md:flex items-center md:order-2 space-x-2 rtl:space-x-reverse">
          <button
            ...
            className="px-3 py-1 border rounded text-white hover:text-amber-400"
          >
            EN
          </button>
          <button
        ...
        >
          <ul className="flex flex-col font-medium p-4 mt-4 border border-gray-100 rounded-lg bg-blue-600 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-transparent dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
            <li>
              <a
                href="/home"
                className="block py-2 px-3 text-white hover:text-amber-400"
              >
                {t("home")}
              </a>
            </li>
            ...
            <li>
              <button
                ...
                className="block py-2 px-3 bg-amber-300 rounded-md text-white hover:bg-amber-600 transition"
              >
                {t("logout") || "Logout"}
              </button>
            </li>
